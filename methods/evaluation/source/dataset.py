...
class Dataset(object):
    ...
    def get_all_void_mask(self,sequence):
        mask_shape = np.array(Image.open(self.sequences[sequence]["masks"][0])).shape
        void_masks = np.zeros((len(self.sequences[sequence]["masks"]), *mask_shape), dtype=np.uint8)
        

        for id,  void_mask in enumerate(self.sequences[sequence]["void_masks"]):
            # print("void_masks:" ,  void_mask)

            void_id = int(void_mask.split('/')[-1].split('.')[0])
            void_masks[void_id, ...] = np.array(Image.open(void_mask))
            print("unique void masks:", np.unique(void_masks[void_id, ...]))
...
class DAVIS2017_Dataset(object):
    SUBSET_OPTIONS = ['train', 'val', 'test']
    VOID_LABEL = 255
    ...
    def __init__(self, root, subset='val', sequences='all'):
        ...
        self.subset = subset
        self.root = root
        self.img_path = os.path.join(self.root,  'JPEGImages', '480p')
        annotations_folder = 'Annotations'
        self.mask_path = os.path.join(self.root, annotations_folder,'480p')
