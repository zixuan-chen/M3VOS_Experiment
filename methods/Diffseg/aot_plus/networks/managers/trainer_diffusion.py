...
class Duffiser_Trainer(Trainer):
    ...
    def prepare_dataset(self):
        ...
        train_datasets = []
        if 'static' in cfg.DATASETS: # 图片数据集
        ...
        if 'test' in cfg.DATASETS:
            test_dataset = TEST( # 看看能不能跑通的测试数据集
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
            )
        ...
        while step < cfg.TRAIN_TOTAL_STEPS:
            ...
            for frame_idx, sample in enumerate(train_loader):
                ...
                all_frames = torch.cat(
                    ...
                    dim=0,
                )   # len_seq * batch_size  , 3 , h , w
                all_labels = torch.cat(
                    [ref_labels, prev_labels] + curr_labels,
                    dim=0,
                )  # len_seq * batch_size  , 1 , h , w
                ...
                if self.rank == 0:
                    ...
                    if step % cfg.TRAIN_TBLOG_STEP == 0:
                        ...
                        self.process_log(
                            ref_imgs, all_f[-2], all_f[-1], # ? all_f[-2], all_f[-1]有点奇怪
