...
class AOTEngine(nn.Module):
    def __init__(
        ...
        self.align_corners = aot_model.cfg.MODEL_ALIGN_CORNERS # 插值的参数
        self.AOT = aot_model 
    ...
    def forward(
        ...
        if hasattr(self.cfg, "PREV_PROBE") and self.cfg.PREV_PROBE:
            ...
            self.total_offline_frame_num = len(self.offline_masks) # len: (seq_len) : (batch_size, h , w)
            
            ref_aux_loss = self.add_reference_frame(
        ...
        if self.cfg.MODEL_DECODER != "diffusion":

            with grad_state():
                ref_aux_loss, ref_aux_mask = self.generate_loss_mask(
                    self.offline_masks[self.frame_step], step)  # 预测的loss
        else:
            ref_aux_mask = None
        ...
        aux_losses = [ref_aux_loss] # aux_loss指的是参考帧的预测loss
        ...
        self.match_propogate_one_frame(
            mask=self.offline_masks[self.frame_step]) # 利用参考帧,预测prev帧的mask
        curr_loss, curr_mask, curr_prob = self.generate_loss_mask(
            self.offline_masks[self.frame_step], step, return_prob=True) # 然后计算prev帧的loss
        # -------------------------------------




        # [modify] 把上面分割线内的改成diffusion的loss:  curr_loss 里面加上一个diffusion的去噪loss,
        ...
        for _ in range(self.total_offline_frame_num - 2):
            ...
            self.match_propogate_one_frame(mask=curr_prob) # 利用上一帧的预测mask值,来预测下一帧的mask
            curr_loss, curr_mask, curr_prob = self.generate_loss_mask(
    ...
    def generate_offline_masks(self, masks):
        offline_one_hot_masks, offline_ignore_masks = one_hot_mask(
            masks, self.max_obj_num)  # offline_one_hot_masks: (seq * batch_s , nax_obj_num + 1, h , w) ; offline_ignore_masks: (seq * batch_s , c(1), h, w)
        self.offline_masks = list( 
            torch.split(masks, self.batch_size, dim=0))   # len: (seq_len)  :(batch_size, 1 , h, w )
        self.offline_one_hot_masks = list(
            torch.split(offline_one_hot_masks, self.batch_size, dim=0))  #  len: (seq_len)  :(batch_size, nax_obj_num + 1 , h, w )
        self.offline_ignore_masks = list(
            torch.split(offline_ignore_masks, self.batch_size, dim=0))  #  len: (seq_len)  :(batch_size, 1 , h, w )
    ...
    def add_reference_frame(
        ...
        if img_embs is None:
            ...
            curr_enc_embs = img_embs # curr_enc_embeds:len:  (short_cut)  : (batch_size,c,  h , w)
        ...
        if self.pos_emb is None:
            ...
            ).view(self.batch_size, -1, self.enc_hw).permute(2, 0, 1)  # h*w. batch_size, fea_dim
        ...
        ) # len : (curr_frame_num??) : (h*w , batch_size, fea_dim)
        ...
        if self.cfg.MODEL_DECODER == "diffusion":
            return  self.decode_current_logits(curr_enc_embs, curr_lstt_output,  gt_mask=self.ref_one_hot_mask_downsample )  # 主要目的:存一下reference_frame的 self.pred_id_logits 

        else:
            return self.decode_current_logits(curr_enc_embs, curr_lstt_output)  # 主要目的:存一下reference_frame的 self.pred_id_logits 
    ...
    def update_short_term_memory(self, curr_mask, curr_id_emb=None, step=0):
        ...
        if self.cfg.REVERSE_INFER:
            ...
            if is_update_long_memory:
                ...
                if self.cfg.MODEL_DECODER == 'diffusion':
                    cur_loss , pred_id_logits = self.decode_current_logits(self.ref_enc_embs, curr_lstt_output , gt_mask = self.ref_mask)
    ...
    def match_propogate_one_frame(self, img=None, img_embs=None, mask=None, output_size=None):
        ...
        return self.decode_current_logits(curr_enc_embs, curr_lstt_output, output_size=output_size, gt_mask= mask)
    

    def decode_current_logits(self, curr_enc_embs, curr_lstt_embs, output_size=None, gt_mask = None):

        if self.cfg.MODEL_DECODER == "diffusion":
            noise_loss , pred_id_logits = self.AOT.decode_id_logits(
                curr_lstt_embs,                                     
                curr_enc_embs,
                gt_mask
            )
        ...
        if self.cfg.MODEL_DECODER == "diffusion":
            return noise_loss, pred_id_logits
    ...
    def calculate_current_loss(self, gt_mask, step):
        ...
        logit_list = []
        for batch_idx, obj_num in enumerate(self.obj_nums):
            now_label = gt_mask[batch_idx].long() #  (seq_len  , h,w )
            now_logit = pred_id_logits[batch_idx, :(obj_num + 1)].unsqueeze(0) # (seq_len, obj_num + 1, h , w ) , 有一个维度表示空 mask
