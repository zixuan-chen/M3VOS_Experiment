...
def eval_vos(cfg: DictConfig):
    ...
    if cfg.weights is not None:
        ...
        # cutie_weights = torch.load("output/cutie-base-mega.pth")
        cutie.load_weights(model_weights)
        # cutie.load_weights(cutie_weights)
    else:
    ...
    aux_output_root = path.join(run_dir, "Aux")
        ...
        try:
            ...
            for ti, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    # print(">>>Forward processing frame: ", frame_name)
                    ...
                    prob, memory_readout = processor.step(image, mask, valid_labels, end=(ti == vid_length - 1))
                    ...
                    if save_all or info['save']:
                        saver.process(prob,
                                      ...
                                      aux=processor.memory.aux,
                                      shape=shape,
                                      last_frame=(ti == vid_length - 1),
                                      path_to_image=path_to_image)
                    
                    if cfg.use_reverse_prop:
                        
                        frames_to_polish -= 1
                        
                        if frames_to_polish == 0 or (ti== vid_length - 1 and slide_window):
                            # start reverse polish
                            processor.backup_state()
                            for rdata in reversed(slide_window):
                                image = rdata['rgb'].cuda()
                                mask = rdata.get('mask')
                                if mask is not None:
                                    mask = mask.cuda()
                                
                                valid_labels = rdata.get('valid_labels')
                                if valid_labels is not None:
                                    valid_labels = valid_labels.tolist()
                                ...
                                resize_needed = info['resize_needed']
                                path_to_image = info['path_to_image']
                                # print(">>>Backward processing frame: ", frame_name)
                                start.record()
                                prob, _ = processor.step(image, mask, valid_labels, 
                                                      backward=True, 
                        ...
                        data['forward_readout'] = memory_readout
                        slide_window.append(data)
                        if len(slide_window) > cfg.reverse_polish.polish_frames:
                            slide_window.pop(0)
            saver.end()
            if is_burst:
