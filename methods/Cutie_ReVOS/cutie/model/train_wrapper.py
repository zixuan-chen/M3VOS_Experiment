    ...
    def forward(self, data: Dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            for ti in range(1, seq_length):
                ...
                # Segment frame ti
                readout, aux_input = self.read_memory(keys[:, :, ti], selections[:, :, ti], ref_keys,
                                                      ref_shrinkages, ref_msk_values, obj_values,
                                                      pix_feat[:, ti], sensory, masks, selector)
                forward_readout[ti] = readout
           ...
           # reverse polish step
            ridx_queue = torch.arange(seq_length)
            for ti in range(seq_length-2, 0, -1):
                # idx = [torch.randperm(seq_length - 1 - ti)[:self.num_ref_frames] for _ in range(b)]
                idx = [torch.randperm(seq_length)[:self.num_ref_frames] for _ in range(b)]
                ridx = [ridx_queue[i] for i in idx]
                ...
                # Segment frame ti
                readout, aux_input = self.read_memory(keys[:, :, ti], 
                                                      selections[:, :, ti], 
                                                      ref_keys,
                                                      ref_shrinkages, 
                                                      ref_msk_values, 
                ...
                readout = self.readout_fuser(readout, forward_readout[ti])
                ...
                if ti > 0:
                    ridx_queue = torch.cat((ridx_queue[1:], torch.tensor([ti])))
                    deep_update = np.random.rand() < self.deep_update_prob
                    msk_val, sensory, obj_val, _ = self.encode_mask(frames[:, ti],
                                                                    pix_feat[:, ti],
                                                                    sensory,
                                                                    masks,
                                                                    deep_update=deep_update)
                    msk_values = torch.cat([msk_values[:, :, :, 1:], msk_val.unsqueeze(3)], 3)
                    obj_values = torch.cat([obj_values[:, :, 1:], obj_val.unsqueeze(2)], 
                                           2) if obj_val is not None else None
