...
class CUTIE(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        self.boost_rate = model_cfg.boost_rate
        ...
        self.readout_fuser = GroupReadoutFuser(model_cfg)
    ...
    def read_memory(self, query_key: torch.Tensor, 
    ...
    def segment(self,
                ...
                *,
                ...
                forward=False) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        """
        ...
        if forward:
            tmp_logits = logits
            logits = self.boost_rate * logits
            
        prob = torch.sigmoid(logits)
        
        tmp_prob = torch.sigmoid(tmp_logits) if forward else None
        

        if selector is not None:
            prob = prob * selector
            tmp_prob = tmp_prob * selector if forward else None
        ...
        if forward:
            tmp_logits = aggregate(tmp_prob, dim=1)
            tmp_logits = F.interpolate(tmp_logits, scale_factor=4, mode='bilinear', align_corners=False)
            tmp_prob = F.softmax(tmp_logits, dim=1)
        return sensory, logits, prob, tmp_prob
