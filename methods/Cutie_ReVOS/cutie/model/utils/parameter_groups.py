...
def get_parameter_groups(model, stage_cfg, print_log=False):
    ...
    for name, param in model.named_parameters():
        ...
        if not inserted:
            ...
            # log.info(f'{name} is counted as other parameter with shape {param.shape}')
...
def get_parameter_groups_readout_fuser(model, stage_cfg, print_log=False):
    """
    Assign different weight decays and learning rates to only "readout_fuser" parameters.
    Returns a parameter group which can be passed to the optimizer.
    """
    base_lr = stage_cfg.learning_rate
    weight_decay = stage_cfg.weight_decay
    ...
    for name, param in model.named_parameters():
        if name.startswith("module.readout_fuser"):
            ...
            readout_fuser_params.append(param)
            if print_log:
