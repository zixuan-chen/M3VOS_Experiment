...
class GroupReadoutFuser(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        super().__init__()
        self.value_dim = model_cfg.value_dim
        out_dim = self.value_dim
        x_transform = GConv2d(out_dim, out_dim, kernel_size=1)
        g_transform = GConv2d(out_dim, out_dim, kernel_size=1)
        self.distributor = MainToGroupDistributor(x_transform=x_transform,
                                                  g_transform=g_transform,
                                                  method='cat')
       

        self.block1 = CAResBlock(2 * out_dim, out_dim)
        self.block2 = CAResBlock(out_dim, out_dim)

    def forward(self, f_readout: torch.Tensor, b_readout: torch.Tensor):
        batch_size, num_objects = f_readout.shape[:2]

        g = self.distributor(f_readout, b_readout, skip_expand=True)
