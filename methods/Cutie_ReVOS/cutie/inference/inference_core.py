...
class InferenceCore:
    ...
    def _segment(self,
                 ...
                 forward_readout: torch.Tensor = None,
                 backward: bool = False,
        ...
        if self.flip_aug:
            # average predictions of the non-flipped and flipped version
            tmp_pred_prob_with_bg = (tmp_pred_prob_with_bg[0] +
                                 torch.flip(tmp_pred_prob_with_bg[1], dims=[-1])) / 2
        else:
            tmp_pred_prob_with_bg = tmp_pred_prob_with_bg[0] if not backward else None

        if update_sensory:
            self.memory.update_sensory(sensory, self.object_manager.all_obj_ids)
        return pred_prob_with_bg, memory_readout, tmp_pred_prob_with_bg
    ...
    def step(self,
             ...
             forward_readout: Optional[torch.Tensor] = None,
             *,
             idx_mask: bool = True,
             end: bool = False,
        ...
        is_mem_frame = ((abs(self.curr_ti - self.last_mem_ti) >= self.mem_every) or
        ...
        # update_sensory = (abs(self.curr_ti - self.last_mem_ti) in self.stagger_ti) and (not end)
        update_sensory = (not end)
        # encoding the image
        ...
        #     print("at 276, last mask is None")
        # else:
        #     print("at 276, last mask shape:", self.last_mask.shape)
        memory_readout = None
        if need_segment:
            pred_prob_with_bg, memory_readout, tmp_pred_prob_with_bg = self._segment(key,
                                              selection,
                                              pix_feat,
                                              ms_feat,
                                              update_sensory=update_sensory,
        ...
        if mask is not None:
            ...
            # print("corresponding_tmp_ids", corresponding_tmp_ids)
            ...
            if need_segment:
                ...
                if not backward:
                    ...
                    mask = torch.stack([mask == obj_id for obj_id in self.object_manager.all_obj_ids], dim=0)

                # print("here generate new mask from correspoinding_tmp_ids")
            pred_prob_with_bg = aggregate(mask, dim=0)
            pred_prob_with_bg = torch.softmax(pred_prob_with_bg, dim=0)
            tmp_pred_prob_with_bg = pred_prob_with_bg
        ...
        if (is_mem_frame or force_permanent) and (not backward):
        ...
        tmp_output_prob = unpad(tmp_pred_prob_with_bg, self.pad) if not backward else None
        if resize_needed:
            ...
            output_prob = F.interpolate(output_prob.unsqueeze(0),
                                        ...
                                        mode='bilinear',
                                        align_corners=False)[0]
            tmp_output_prob = F.interpolate(tmp_output_prob.unsqueeze(0),
                                        size=(h, w),
                                        mode='bilinear',
                                        align_corners=False)[0] if not backward else None

        return output_prob, memory_readout
    ...
    def backup_state(self):
        self.curr_ti_last_forward = self.curr_ti
        
        if self.object_manager.all_obj_ids:
            self.sensory_backup = self.memory.get_sensory(self.object_manager.all_obj_ids)
        else:
            self.sensory_backup = None

        self.last_mask_backup = self.last_mask
    
    def restore_state(self):
        self.curr_ti = self.curr_ti_last_forward
        if self.sensory_backup is not None:
            self.memory.update_sensory(self.sensory_backup, self.object_manager.all_obj_ids)
        self.last_mask = self.last_mask_backup