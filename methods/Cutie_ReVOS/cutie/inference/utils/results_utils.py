...
class ResultSaver:
    def __init__(self,
        ...
        self.aux_output_root = aux_output_root
...
def save_result(queue: Queue):
    while True:
        ...
        if saver.save_aux and aux:
            this_out_path = path.join(saver.aux_output_root, saver.video_name)
            os.makedirs(this_out_path, exist_ok=True)
            aux_save = {}
            if 'p_weights' in aux:
                aux_save['p_weights'] = aux['p_weights'].detach().cpu()
            if 'q_weights' in aux:
                aux_save['q_weights'] = aux['q_weights'].detach().cpu()
            if "attn_mask" in aux:
                aux_save['attn_mask'] = aux['attn_mask'].detach().cpu()
            if "q_logits" in aux:
                for i, q_logits_block_i in enumerate(aux['q_logits']):
                    aux_save[f"q_logits_{i}"] = q_logits_block_i.detach().cpu()
            aux_file_name = f'{frame_name[:-4]}.pt'
            if not forward:
                aux_file_name = f'{frame_name[:-4]}_b.pt'
            torch.save(aux_save, path.join(this_out_path, aux_file_name))
        queue.task_done()
