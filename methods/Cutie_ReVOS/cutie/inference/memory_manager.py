...
class MemoryManager:
    ...
    def __init__(self, cfg: DictConfig, object_manager: ObjectManager):
        ...
        self.use_reverse_prop = cfg.use_reverse_prop
        ...
        if self.use_reverse_prop:
            self.reverse_mem = KeyValueMemoryStore(save_selection=self.use_long_term,
                                            save_usage=self.use_long_term)
    ...
    def _get_visual_values_by_ids(self, obj_ids: List[int]) -> torch.Tensor:
        ...
        if self.use_reverse_prop and obj_ids[0] in self.reverse_mem.value:
            rs_value = torch.stack([self.reverse_mem.value[obj] for obj in obj_ids], dim=1)
            value = torch.cat([rs_value, value], dim=-1)
    ...
    def read(self, pix_feat: torch.Tensor, query_key: torch.Tensor, selection: torch.Tensor,
        ...
        for bucket_id, bucket in buckets.items():
            if self.use_long_term and self.long_mem.engaged(bucket_id):
                ...
                if self.use_reverse_prop and self.reverse_mem.engaged(bucket_id):
                    memory_key = torch.cat([self.work_mem.key[bucket_id], self.reverse_mem.key[bucket_id]], -1)
                    shrinkage = torch.cat([self.work_mem.shrinkage[bucket_id], self.reverse_mem.shrinkage[bucket_id]], -1)
            ...
            for objects in object_chunks:
                ...
                readout_memory, aux_features = network.readout_query(pixel_readout, this_obj_mem, need_weights=self.need_weights)
