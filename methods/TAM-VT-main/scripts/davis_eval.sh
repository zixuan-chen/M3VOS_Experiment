...
export OUTPUT_DIR_CHECKPOINT="checkpoints/pretrain_vost/davis_eval_davis"


python main_davis_memory.py \
    --config_path ./config/davis_multi_scale_memory.yaml  \
    ema=True \
    resume=checkpoints/tamvt_vost.pth \
    val_tasks.names="['davis']"  \
    ...
    data.davis.use_test_transform=False \
    ...
    eval_flags.davis.eval_first_window_only=False \
    eval_flags.davis.vis_only_no_cache=True  \
    eval_flags.davis.vis_only_pred_mask=True \
    backbone=resnet50 \
